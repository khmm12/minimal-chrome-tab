...
describe('asGetters', () => {
  it('creates object with reactive fields from accessors', () => {
    ...
    expect(accessors.a).toHaveBeenCalledTimes(1)
    expect(accessors.b).toHaveBeenCalledTimes(1)

    expect(getters.a).toBe(1) // read
    expect(accessors.a).toHaveBeenCalledTimes(2)
    expect(accessors.b).toHaveBeenCalledTimes(1)

    accessors.a.mockReturnValue(3)

    expect(getters.a).toBe(3) // read
    expect(accessors.a).toHaveBeenCalledTimes(3)
    expect(accessors.b).toHaveBeenCalledTimes(1)
  })
})
